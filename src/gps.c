/*
 * $Id: gps.c,v 1.1.1.1 2002/02/02 03:17:32 kg4ijb Exp $
 *
...
int decode_gps_rmc(char *data, char *long_pos, int long_pos_length, char *lat_pos, int lat_pos_length, char *spd, char *unit, int unit_length, char *cse) {
    char *temp_ptr;
...
// and check for end of input while tokenizing.
    ...
    if (strncmp(data,"$GPRMC,",7)==0) {
        if(strchr(data,',')!=NULL) {
            ...
            (void)strtok(NULL,",");   /* get time and skip it */
...
int decode_gps_gga(char *data, char *long_pos, int long_pos_length, char *lat_pos, int lat_pos_length, char *sats, char *alt) {
    char *temp_ptr;
    char temp_data[MAX_TNC_LINE_SIZE+1];    // Big in case we get concatenated packets (it happens!)
    char long_pos_x[10];
    char long_ew;
...
void gps_data_find(char *gps_line_data, int port) {
    ...
    if (got_gps_gpgga==1 && got_gps_gprmc==1){ /* decode data and shutdown gps port for timed interval */
        if (!gps_stop_now) {
            gps_stop_now=1;
            if (decode_gps_rmc(gps_gprmc,long_pos,sizeof(long_pos),lat_pos,sizeof(lat_pos),spd,sunit,sizeof(sunit),cse)==1) { /* mod station data */
                /*printf("RMC <%s> <%s><%s> %c <%s>\n",long_pos,lat_pos,spd,sunit[0],cse);*/
                if (decode_gps_gga(gps_gpgga,long_pos,sizeof(long_pos),lat_pos,sizeof(lat_pos),sats,alt)==1) {     /* mod station data */
